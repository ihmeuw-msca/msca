...
from msca.typing import NDArray
...
class Expit(C2Fun):
    ...
    def d2fun(x: NDArray) -> NDArray:
        ...
        """
        z = np.where(x > 0, np.exp(-x), np.exp(x))
        y = np.where(x > 0, z**2 - z, z - z**2) / (1 + z) ** 3
        return y
...
class Logerfc(C2Fun):
    ...
    def fun(x: NDArray) -> NDArray:
        ...
        y = np.where(
            x < 25,
            np.log(erfc(x)),
            -(x**2) + np.log(1 - 0.5 / x**2) - np.log(np.sqrt(np.pi) * x),
        )
        return y
    ...
    def dfun(x: NDArray) -> NDArray:
        ...
        y = np.where(
            x < 25,
            -2 * np.exp(-(x**2)) / (erfc(x) * np.sqrt(np.pi)),
            -2 * x - 1 / x + 2 / (2 * x**3 - x),
        )
        return y
...
.. code-block:: python
    ...
    from msca.c2fun.numpy import c2fun_dict
